...
export function createRecipeCards(recipes, container) {
    ...
    waitForSelector(containerName).then((gridContainer) => {
...
export async function bindUserProfile(profile) {
    ...
    router.addPage(pageName, function () {
        ...
        document.getElementById("userEmail").textContent = profile.email
            ? profile.email
            : "OliveU Email";
        document.getElementById("notProfile").src = profile.imageURL
            ? profile.imageURL
        ...
        if (profile.myRecipe) {
            createRecipeCards(
                profile.myRecipe,
                ".myUploadRecipeCardGridContainer"
            );
        }
        if (profile.savedRecipe) {
            createRecipeCards(
                profile.savedRecipe,
                ".mySavedRecipeCardGridContainer"
            );
        }
    });
