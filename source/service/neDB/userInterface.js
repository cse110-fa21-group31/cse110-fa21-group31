...
async function removeMyRecipe(userDB, id, recipeId) {
  ...
  if(recipeSaved(data.myRecipe, recipeId)){
    ...
    data.myRecipe = data.myRecipe.filter(rep => 
      rep != recipeId 
    )
    // update in db
    data = new Promise((resolve, reject) => {
...
async function unsaveRecipe(userDB, id, recipeId) {
  ...
  if(recipeSaved(data.savedRecipe, recipeId)){
    // remove the recipe from array
    data.savedRecipe = data.savedRecipe.filter(rep => 
      rep != recipeId 
    )
    // update in db
    data = new Promise((resolve, reject) => {
...
module.exports = {createUser, hasUser, getUser, addMyRecipe, removeMyRecipe, saveRecipe, unsaveRecipe};